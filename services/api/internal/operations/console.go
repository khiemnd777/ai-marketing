...
func safeConfiguration(cfg config.Config) map[string]any {
	return map[string]any{"environment": cfg.Environment, "demoMode": cfg.DemoMode, "openai": map[string]any{"configured": cfg.OpenAI.Validate() == nil, "model": cfg.OpenAI.Model, "baseUrl": safeHost(cfg.OpenAI.BaseURL)}, "seedance": map[string]any{"configured": cfg.Seedance.Validate() == nil, "model": cfg.Seedance.Model, "apiVersion": cfg.Seedance.APIVersion}, "meta": map[string]any{"configured": cfg.Meta.Validate() == nil, "apiVersion": cfg.Meta.APIVersion}, "renderer": map[string]any{"configured": cfg.Renderer.BaseURL != "" && cfg.Renderer.SharedSecret != "", "baseUrl": safeHost(cfg.Renderer.BaseURL)}, "storage": map[string]any{"configured": cfg.R2.Validate() == nil, "bucket": cfg.R2.Bucket}}
}
