...
- Added a keyboard-accessible `Xuất PDF` action that opens the browser's save/print dialog with a stable suggested filename. Print-only styling removes the application shell and produces three A4 landscape pages for package cards, comparison, and commercial terms.
...
- Passed all 52 Web tests, strict Web typecheck, ESLint, the Next.js production build, and `git diff --check`. Browser QA passed at 1440×1000 and 390×844: three-to-one-column responsive layout, clearly separated monthly and one-time onboarding prices, correct free/50%-off onboarding offers, a 44px export control, keyboard activation, restored page title after export, locally scrollable comparison table without page overflow, active navigation, and mobile drawer behavior. The generated three-page PDF was rendered to PNG and visually verified with all required catalogue text extractable and no application navigation included.
