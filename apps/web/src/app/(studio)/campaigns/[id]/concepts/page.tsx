...
function ConceptCard({ item, scope, onChanged }: { item: Concept; scope: ReturnType<typeof useCampaignRoute>; onChanged: () => Promise<unknown> }) {
  ...
  return <Card className="p-6"><div className="mb-5 flex flex-wrap items-center gap-2"><Badge tone={locked ? "good" : item.status === "REJECTED" ? "danger" : "warn"}>{item.status}</Badge><Badge>{draft.videoFormat}</Badge><span className="ml-auto text-xs text-[var(--muted)]">v{item.currentVersion} · ${draft.estimatedCostUsd.toFixed(4)}</span></div>
    <div className="grid gap-4"><Field label="Tên concept"><input className={inputClass} disabled={locked} value={draft.title} onChange={(e) => change("title", e.target.value)} /></Field><Field label="Hook"><textarea className={textareaClass} disabled={locked} value={draft.hook} onChange={(e) => change("hook", e.target.value)} /></Field><Field label="Thông điệp cốt lõi"><textarea className={textareaClass} disabled={locked} value={draft.coreMessage} onChange={(e) => change("coreMessage", e.target.value)} /></Field><Field label="Phù hợp khán giả"><textarea className={textareaClass} disabled={locked} value={draft.audienceFit} onChange={(e) => change("audienceFit", e.target.value)} /></Field><div className="grid grid-cols-2 gap-3"><Field label="Số cảnh"><input className={inputClass} type="number" min={2} max={8} disabled={locked} value={draft.expectedSceneCount} onChange={(e) => change("expectedSceneCount", Number(e.target.value))} /></Field><Field label="Giây Seedance"><input className={inputClass} type="number" min={6} disabled={locked} value={draft.expectedSeedanceSeconds} onChange={(e) => change("expectedSeedanceSeconds", Number(e.target.value))} /></Field></div><Field label="Bối cảnh"><input className={inputClass} disabled={locked} value={draft.environment} onChange={(e) => change("environment", e.target.value)} /></Field><Field label="Product placement"><input className={inputClass} disabled={locked} value={draft.productPlacement} onChange={(e) => change("productPlacement", e.target.value)} /></Field></div>
    {(update.error || decide.error) ? <p className="mt-4 text-sm text-[var(--coral)]">{update.error?.message ?? decide.error?.message}</p> : null}<div className="mt-5 flex flex-wrap gap-2">{!locked ? <Button className="bg-white text-[var(--ink)] ring-1 ring-[var(--line)]" disabled={update.isPending} onClick={() => update.mutate()}><Save className="mr-2 size-4" />Lưu</Button> : null}{item.status === "DRAFT" || item.status === "REJECTED" ? <Button disabled={decide.isPending} onClick={() => decide.mutate("APPROVE")}><Check className="mr-2 size-4" />Duyệt</Button> : null}{item.status === "APPROVED" ? <Button disabled={decide.isPending} onClick={() => decide.mutate("LOCK")}><LockKeyhole className="mr-2 size-4" />Khóa concept</Button> : null}{!locked && item.status !== "REJECTED" ? <Button className="bg-[var(--coral)]" disabled={decide.isPending} onClick={() => decide.mutate("REJECT")}><X className="mr-2 size-4" />Từ chối</Button> : null}</div>
  </Card>;
}
