...
function AssetEditor({ asset, scope, onClose, onSaved }: { asset: Asset; scope: MediaScope; onClose: () => void; onSaved: () => Promise<unknown> }) {
  ...
  return <Card className="mb-6 border-[var(--moss)] p-6"><div className="mb-5 flex items-center justify-between"><div><h2 className="font-serif text-xl font-bold">Sửa metadata</h2><p className="mt-1 text-sm text-[var(--muted)]">{asset.name} · optimistic version {asset.version}</p></div><button aria-label="Đóng trình sửa" className="rounded-full p-2 hover:bg-[#edf0e7]" onClick={onClose}><X className="size-5" /></button></div><div className="grid gap-4 md:grid-cols-2"><Field label="Tên"><input className={inputClass} value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} /></Field><Field label="Danh mục"><select className={inputClass} value={form.category} onChange={(event) => setForm((current) => ({ ...current, category: event.target.value }))}>{includeCurrentOption(mediaCategoryOptions,form.category).map((option)=><option key={option.value} value={option.value}>{option.label}</option>)}</select></Field><Field label="Folder"><input className={inputClass} value={form.folder} onChange={(event) => setForm((current) => ({ ...current, folder: event.target.value }))} /></Field><Field label="Tags (phân cách dấu phẩy)"><input className={inputClass} value={form.tags} onChange={(event) => setForm((current) => ({ ...current, tags: event.target.value }))} /></Field><Field label="Ngày hết hạn"><input className={inputClass} type="date" value={form.expiresAt} onChange={(event) => setForm((current) => ({ ...current, expiresAt: event.target.value }))} /></Field><div className="md:col-span-2"><Field label="Quyền sử dụng"><textarea className={textareaClass} value={form.usageRights} onChange={(event) => setForm((current) => ({ ...current, usageRights: event.target.value }))} /></Field></div></div>{save.error ? <p role="alert" className="mt-3 text-sm text-[var(--coral)]">{save.error.message}</p> : null}<div className="mt-5 flex gap-3"><Button disabled={save.isPending || !form.name.trim() || !form.usageRights.trim()} onClick={() => save.mutate()}><Check className="mr-2 size-4" />Lưu metadata</Button><Button className="bg-white text-[var(--ink)] ring-1 ring-[var(--line)]" onClick={onClose}>Hủy</Button></div></Card>;
}
