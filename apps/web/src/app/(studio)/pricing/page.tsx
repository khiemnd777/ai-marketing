  ...
  return (
    <>
      <PageHeader
        ...
        action={<Badge tone="warn">Áp dụng từ 22/08/2026</Badge>}
      />

      <section aria-labelledby="pricing-packages-heading">
      ...
      <section className="mt-8 grid gap-6 xl:grid-cols-[1.1fr_.9fr]" aria-labelledby="commercial-notes-heading">
...
function PackageCard({ item }: { item: ServicePackage }) {
  return (
    <Card className={item.recommended ? "relative flex h-full flex-col border-[var(--moss)] bg-[#fbfff1] p-6 ring-2 ring-[var(--moss)]/10 md:p-7" : "flex h-full flex-col p-6 md:p-7"}>
      ...
      <div className="mt-5 border-y border-[var(--line)] py-5">
        ...
        <p className="mt-1 text-sm font-semibold text-[var(--muted)]">mỗi tháng · phí khởi tạo {formatVnd(item.onboardingFeeVnd)} một lần</p>
      </div>
      ...
      <div className="mt-6 flex items-center gap-2 rounded-2xl bg-[#edf0e7] px-4 py-3 text-xs font-semibold text-[var(--muted)]"><BarChart3 className="size-4 shrink-0" />{item.reviewCadence}</div>
    </Card>
  );
}
