...
func New(cfg config.Config, logger *slog.Logger, pool *pgxpool.Pool) (*fiber.App, error) {
	...
	protected.Post("/clients/:clientId/workspaces/:workspaceId/analytics/recommendations/generate", auth.RequireRole(db.InternalUserRoleADMIN, db.InternalUserRoleOPERATOR), analyticsHandler.GenerateRecommendations)
