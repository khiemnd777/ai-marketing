...
export function GenerationPanel({ operation }: { operation: GenerationOperation }) {
  ...
  return <Card className="mb-6 flex flex-col gap-4 p-5 md:flex-row md:items-center">
    ...
    <Button disabled={start.isPending || running} onClick={() => start.mutate()}>{running ? <><RefreshCw className="mr-2 size-4 animate-spin" />Đang xử lý</> : `Tạo ${operation.toLowerCase()}`}</Button>
  </Card>;
}
