...
2. Configure the required variables from `.env.example` in Coolify secrets. Use a TLS-verified `DATABASE_URL`; random 32-byte decoded key material for session/encryption settings; a 32+ character renderer secret; private R2 credentials; and restricted provider credentials. No secret uses a `NEXT_PUBLIC_` prefix.
...
5. Verify `/v1/health/live`, `/v1/health/ready`, login/CSRF, provider status, renderer readiness, River queues, Prometheus targets, Tempo traces, a signed media read, and a demo-mode smoke workflow before enabling live providers.
