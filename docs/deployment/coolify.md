...
3. Set `APP_DOMAIN`, `APP_URL=https://<domain>`, Meta redirect/callback URLs, and Caddy DNS/ports. Set `BROWSER_STORAGE_ORIGINS` to the space-separated browser-facing origins used by configured R2/S3 endpoints (the production default allows `https://*.r2.cloudflarestorage.com`). Restrict River UI and Grafana with SSO/VPN or network policy in addition to their local authentication.
