...
import { buildRenderPlan, captionBottom, decibelGain, overlayPlacement } from "./plan.js";
...
function OverlayLayer({ props }: { props: StudioRenderProps }) {
  ...
  return <AbsoluteFill>{props.manifest.overlays.filter((overlay) => frame >= overlay.startFrame && frame < overlay.endFrame).map((overlay, index) => {
    const { bottom, fontSize, prominent } = overlayPlacement(overlay.type, overlay.safeZone);
    ...
    return <div key={`${overlay.type}-${index}`} style={{ position: "absolute", left: 56, right: overlay.type === "disclaimer" ? 56 : 220, bottom, color: "white", fontFamily: "Arial, 'Noto Sans', sans-serif", fontSize, fontWeight: prominent ? 800 : 600, lineHeight: 1.15, padding: prominent ? "20px 28px" : "12px 20px", background: "rgba(10,20,18,0.76)", borderRadius: 20, textShadow: "0 2px 8px rgba(0,0,0,.5)" }}>{overlay.value}</div>;
  })}</AbsoluteFill>;
}
...
function CaptionLayer({ manifest }: { manifest: RenderManifest }) {
  ...
  if (!cue) return null;
  return <div style={{ position: "absolute", left: 90, right: 90, bottom: captionBottom(manifest, frame), textAlign: "center", color: "white", fontFamily: "Arial, 'Noto Sans', sans-serif", fontWeight: 800, fontSize: 44, lineHeight: 1.25, padding: "18px 26px", borderRadius: 20, background: "rgba(0,0,0,.72)", textShadow: "0 2px 6px black" }}>{cue.text}</div>;
}

export function StudioFinalVideo(props: StudioRenderProps) {
  const { fps } = useVideoConfig();
  const plan = buildRenderPlan(props.manifest);
  return <AbsoluteFill style={{ backgroundColor: "#101714" }}>{props.manifest.scenes.map((scene, index) => {
    const timing = plan.scenes[index]!;
    return <Sequence key={scene.sceneId} from={timing.from} durationInFrames={timing.durationInFrames}><SceneClip scene={scene} source={props.assetUrls[scene.source.objectKey]!} products={scene.productMedia.map((asset) => ({ url: props.assetUrls[asset.objectKey]!, contentType: asset.contentType }))} /></Sequence>;
  })}{props.manifest.music ? <Audio src={props.assetUrls[props.manifest.music.objectKey]!} volume={(frame) => { const now = frame / fps * 1000; const dialogue = props.manifest.captions.some((cue) => now >= cue.startMs && now < cue.endMs); return decibelGain(props.manifest.musicGainDb + (dialogue ? props.manifest.dialogueDuckingDb : 0)); }} loop /> : null}{props.manifest.soundEffects.map((effect) => <Sequence key={`${effect.source.objectKey}-${effect.startMs}`} from={Math.round(effect.startMs / 1000 * fps)}><Audio src={props.assetUrls[effect.source.objectKey]!} volume={decibelGain(effect.gainDb)} /></Sequence>)}<OverlayLayer props={props} />{props.manifest.burnCaptions ? <CaptionLayer manifest={props.manifest} /> : null}</AbsoluteFill>;
}
