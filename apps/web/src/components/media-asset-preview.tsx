...
export function MediaAssetPreview({ asset, scope, className = "aspect-video", fit = "cover" }: { asset: Asset; scope: MediaScope; className?: string; fit?: "cover" | "contain" }) {
  ...
  return <div className={`relative grid place-items-center overflow-hidden bg-[var(--ink)] text-white/60 ${className}`}>{preview.data && isImage ? <Image fill unoptimized sizes="(min-width: 1280px) 33vw, (min-width: 768px) 50vw, 100vw" className={fit === "contain" ? "object-contain p-4" : "object-cover"} src={preview.data.url} alt={`Preview ${asset.name}`} /> : preview.data && isVideo ? <video aria-label={`Preview ${asset.name}`} className="h-full w-full object-cover" src={preview.data.url} muted playsInline preload="metadata" /> : asset.assetType === "AUDIO" ? <Music2 className="size-10" /> : <ImageIcon className="size-10" />}</div>;
}
